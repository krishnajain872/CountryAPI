// ========================================
// FILE: cmd/server/main.go (Render-ready)
// ========================================
package main
...
func main() {
	...
	// -----------------------------
	// 7️⃣ Determine server port (Render-ready)
	// -----------------------------
	port := os.Getenv("PORT")
	if port == "" {
		// fallback to .env config for local dev
		port = cfg.Server.Port
	}

	server := &http.Server{
		Addr:         fmt.Sprintf(":%s", port), // bind all interfaces
		Handler:      router,
